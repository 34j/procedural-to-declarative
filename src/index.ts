...
export interface TaskBase<TType extends ('constant' | 'func' | 'any' | 'declarative') = 'constant' | 'func' | 'any' | 'declarative'> {
  ...
  calledBy?: TaskProcedural<any> | undefined
}
 ...
 */
export interface TaskProcedural<TNumber extends number = number> extends TaskBase<'func'> {
 ...
 */
export function runProcedural<TNumber extends number = number>(track: Track<TNumber>, f: ProceduralFunction<TNumber>): TaskProcedural<TNumber> {
  if (track.isMaterialized)
    throw new Error('Cannot run a procedural function while materialized.')
  const task: TaskProcedural<TNumber> = { type: 'func', f, isSuspended: false, done: false }
 ...
 */
export function toVisibleFrames<TNumber extends number>(frames: ReadonlyArray<TrackMaterialized<TNumber>>): ReadonlyArray<{ time: TNumber, refValues: ReadonlyMap<'Ref' | 'Unknown', Readonly<any>> }> {
 ...
 */
export function useCompiled<TNumber extends number>(track: Track<TNumber>, frames: ReadonlyArray<TrackMaterialized<TNumber>>, time: TNumber): void {
  if (track.isCompilingOrEvaluating)
